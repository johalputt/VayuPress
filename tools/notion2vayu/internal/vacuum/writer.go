...
func Open(path string) (*Writer, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS notion2vayu_checkpoint(
...
func (w *Writer) SaveCheckpoint(ctx context.Context, lastID string) error {
	_, err := w.db.ExecContext(ctx,
		`INSERT OR REPLACE INTO notion2vayu_checkpoint(key,val) VALUES('last_id',?)`,
		lastID)
	return err
}
...
func (w *Writer) LoadCheckpoint(ctx context.Context) (string, error) {
	...
	err := w.db.QueryRowContext(ctx,
		`SELECT val FROM notion2vayu_checkpoint WHERE key='last_id'`).Scan(&val)
...
func (w *Writer) QueryArticles(ctx context.Context) (*sql.Rows, error) {
