// Package vacuum writes converted WordPress posts into a VayuPress SQLite database.
package vacuum
...
// Open opens (or creates) the VayuPress SQLite database at path. It ensures the
// articles table exists (mirroring VayuPress migration 001-baseline) so the tool
// also works against a brand-new database file.
func Open(path string) (*Writer, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS wp2vayu_checkpoint(
...
// Insert writes an article. It returns (true, nil) when a new row was inserted
// and (false, nil) when a row with the same id or slug already existed (skipped).
// The single INSERT OR IGNORE keeps the write path to one statement per post.
...
// SaveCheckpoint records the last processed WordPress post id so an interrupted
// migration can resume from exactly where it stopped.
func (w *Writer) SaveCheckpoint(ctx context.Context, lastID string) error {
	_, err := w.db.ExecContext(ctx,
		`INSERT OR REPLACE INTO wp2vayu_checkpoint(key,val) VALUES('last_id',?)`,
		lastID)
	return err
}

// LoadCheckpoint returns the last processed WordPress post id ("" if none).
func (w *Writer) LoadCheckpoint(ctx context.Context) (string, error) {
	var val string
	err := w.db.QueryRowContext(ctx,
		`SELECT val FROM wp2vayu_checkpoint WHERE key='last_id'`).Scan(&val)
