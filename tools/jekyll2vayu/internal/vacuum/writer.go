...
func Open(dbPath string) (*Writer, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS jekyll2vayu_checkpoint(
...
func (w *Writer) InsertArticle(doc *jekyllparse.Document) (bool, error) {
	id := newID()
...
func (w *Writer) GetCheckpoint() (string, error) {
	var val string
	err := w.db.QueryRow(
		`SELECT val FROM jekyll2vayu_checkpoint WHERE key='last_file'`).Scan(&val)
...
func (w *Writer) SetCheckpoint(path string) error {
	_, err := w.db.Exec(
		`INSERT OR REPLACE INTO jekyll2vayu_checkpoint(key,val) VALUES('last_file',?)`,
		path)
	if err != nil {
