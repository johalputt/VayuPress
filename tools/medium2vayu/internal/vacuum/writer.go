...
func Open(path string) (*Writer, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS medium2vayu_checkpoint(
...
// SaveCheckpoint records the last processed Ghost post id so an interrupted
...
func (w *Writer) SaveCheckpoint(ctx context.Context, lastID string) error {
	_, err := w.db.ExecContext(ctx,
		`INSERT OR REPLACE INTO medium2vayu_checkpoint(key,val) VALUES('last_id',?)`,
		lastID)
	return err
}

// LoadCheckpoint returns the last processed Ghost post id ("" if none).
func (w *Writer) LoadCheckpoint(ctx context.Context) (string, error) {
	var val string
	err := w.db.QueryRowContext(ctx,
		`SELECT val FROM medium2vayu_checkpoint WHERE key='last_id'`).Scan(&val)
