...
# =============================================================================
# Provision the dedicated, CDN-proxy-off relay subdomain so the mobile app's
# long-lived SSE stream reaches the origin unchallenged (see the script header for
# the full rationale). Delegated to setup-talk-subdomain.sh so deploy AND update
# share one implementation. Idempotent and non-fatal: with talk.<domain> DNS not
# yet pointed it skips cleanly and the app keeps using the main domain.
TALK_SETUP="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)/setup-talk-subdomain.sh"
if [[ -f "$TALK_SETUP" ]]; then
  DOMAIN="$DOMAIN" EMAIL="$EMAIL" CACHE_DIR="$CACHE_DIR" run bash "$TALK_SETUP" || \
    warn "VayuTalk subdomain setup reported an issue — the app will use ${DOMAIN} until it is resolved."
else
...
# provisioned until the operator approves them (VayuOS → Domains → "Sync now").
# Idempotent and non-fatal: a domain whose DNS isn't pointed here is skipped
# cleanly. Each secondary gets its OWN cert (--cert-name), never sharing the
# primary's lineage, so the Let's Encrypt 100-SAN cap can never be hit.
