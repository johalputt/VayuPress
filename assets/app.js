...
STATIC_DIR=./static VAYU_DOCS_DIR=./docs ./vayupress --port 8080`,
    ...
    features: [
      { icon:'🪟', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.45)',
        title:'VayuOS — the one and only admin at /os',
        desc:'As of v1.6.0 there is exactly one admin: VayuOS at /os. The old Admin v2 surface is fully removed (ADR-0069 Stage 3) — its handlers, assets and login are gone — and the legacy /admin, /admin/v2 and /admin/v3 paths now issue permanent (301) redirects into the /os equivalent. The block editor owns every authoring flow: native create, lossless auto-import of legacy HTML into blocks on open (non-destructive until save), opt-in AI-assist (continue, rewrite, summarise) with an inline Accept/Discard overlay, and an inline version-history diff. The Theme Studio is native and a dedicated Governance panel surfaces system mode, error budgets and live policy evaluation. The operator consoles — System Modes, Policy Provenance Inspector, Runtime Topology, Replay Explorer, Fault Engine and the ADR Registry — now render inside the single VayuOS shell under /os/* (an Operations sidebar section), so there is no separate operator panel. Still a sovereign single binary — zero CDNs, strict CSP with per-request nonces, no unsafe-eval, no unsafe-inline.',
        tags:['/os','AI-assist','governance'] },
      { icon:'🎬', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.45)',
