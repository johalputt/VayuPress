...
func osConnectorConnectCard(endpoint string) string {
  ...
  <div class="settings-block-title text-sm mt-4">Behind Cloudflare or another CDN / WAF? (common gotcha)</div>
  ...
  <p class="field-hint mt-2">Verify with a plain <code>curl</code> of your site's <code>/health</code> endpoint: it must return JSON (the version), <strong>not</strong> a CDN challenge page. When curl gets through, Claude's backend will too.</p>
</div>`
}
