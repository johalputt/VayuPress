...
// line, from the same kernel answers the chip reads; the kernel's full wording
// sits in the tip beside it.
func veilSelfHeadline(s vayuveil.SelfHardening) string {
	switch {
	case !s.Supported:
		return "This platform offers no way to refuse being dumped."
	case !s.Known:
		return "Unverified: the kernel did not say whether this process can be dumped."
	case s.Undumpable:
		return "This process cannot be dumped, and no other process of its user can read its memory."
	default:
		return "This process can be dumped."
	}
}

func veilCoreHeadline(s vayuveil.SelfHardening) string {
	switch {
	case !s.CoreLimitKnown:
		return "Unverified: the core-file limit could not be read."
	case s.CoreLimitZero:
		return "The kernel writes no core file for this process."
	default:
