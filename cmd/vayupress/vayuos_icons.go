...
// vayuos_icons.go — the Still Air icon set.
//
// One set, drawn to one rule: a 20-unit grid, 1.5 stroke, round caps and joins,
// no fills. Every icon the Still Air shell shows comes from this map; an icon
// from another library, or an emoji standing in for one, does not belong in
// the chrome (TestStillAirChromeUsesOnlyTheIconSet).
...
// saIcon renders one icon from the set. An unknown name is a programming error
// that must not reach a page silently, so it renders the "missing" mark the
// chrome test looks for.
func saIcon(name string) string {
	p, ok := saIcons[name]
	if !ok {
		return `<svg class="sa-ico sa-ico--missing" viewBox="0 0 20 20" aria-hidden="true"></svg>`
	}
	return `<svg class="sa-ico" viewBox="0 0 20 20" aria-hidden="true">` + p + `</svg>`
}

// saIconSized draws an icon that styles itself, for a page that carries no
// console stylesheet (the maintenance page, the offline page, the connector
// consent screen): the look saIcon gets from CSS is spelled out as attributes.
func saIconSized(name string, px int) string {
	sz := strconv.Itoa(px)
	return `<svg width="` + sz + `" height="` + sz + `" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">` + saIcons[name] + `</svg>`
}
...
var saSprite = func() string {
	...
	for _, n := range names {
		b.WriteString(`<symbol id="sa-i-` + n + `" viewBox="0 0 20 20">` + saIcons[n] + `</symbol>`)
	}
