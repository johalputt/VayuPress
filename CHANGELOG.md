...
### Security
- **The supported-versions policy in `SECURITY.md` now describes reality.** It named the `1.0.x`
  series and an `LTS 1.x` that does not exist, while the project shipped 3.15.48 — which reads as
  neglect to anyone evaluating whether to deploy this, and is worse than saying nothing. Replaced
  with the real window: current minor in full support, previous minor critical-only with an end
  date, older unsupported.

  It also states plainly that the Cyber Resilience Act's five-year default support period **is
  not met**, because a single-maintainer project funded by donations should not claim a term it
  cannot honour — a promise like that fails at exactly the moment someone relies on it. What is
  offered instead does not depend on anyone's goodwill: signed tags, an irrevocable Apache-2.0
  grant, and a per-release SBOM, so an operator needing a longer horizon can maintain a fork with
  no permission required and no vendor to negotiate with. Anyone who needs a contractual support
  term is told outright that this project does not meet it.
- **A CycloneDX SBOM is attached to every GitHub Release** and cosign-signed, in both
  `release.yml` and `tag-release.yml`. One was already generated by `sbom.yml`, but only as a CI
  artefact that is garbage-collected with its run — and an SBOM exists to answer "what was in the
  version I deployed", which an expired one cannot. It is signed because an unsigned inventory
  beside a signed binary is a gap in itself: swap the SBOM and the artefact still verifies.
- **`/.well-known/security.txt`** (RFC 9116) published at vayupress.com, listing the security
  mailbox and the GitHub private-advisory route. RFC 9116 requires an `Expires` field and
  scanners report an expired file as stale, so a committed date would have become a finding
  rather than prevented one; `deploy-site.yml` stamps it 180 days out at deploy time and already
  runs daily, so it refreshes itself indefinitely. The assemble step fails loudly if the file is
  missing rather than publishing a site whose advertised security contact 404s.
