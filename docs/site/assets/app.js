...
STATIC_DIR=./static VAYU_DOCS_DIR=./docs ./vayupress --port 8080`,
    ...
    features: [
      { icon:'⬆️', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.55)',
        title:'Update & Backup — one-click updates + full site backup (v1.18.0)',
        desc:'Update VayuPress and back up your entire site without ever touching a shell. From the new Update & Backup page in VayuOS, install the latest signed release in one click — the download is verified by SHA-256 checksum and Ed25519 signature against your pinned release key, your database is backed up automatically, the binary is swapped atomically, and the service re-launches itself to finish. Roll back to the previous binary just as easily. Download your whole site — database and every setting — as a single consistent, checksummed .tar.gz, and restore it here or on another server: imports are validated, your current database is backed up first, then the restored data is swapped in atomically at startup. Export and import stream in constant memory with the request timeouts lifted, so there is no size limit. Every action is admin-gated, CSRF-protected, and audit-logged (ADR-0089).',
        tags:['one-click signed update','full backup / restore','no size limit'] },
      { icon:'🎨', iconBg:'bg-pink-900/60 border border-pink-800/60',    orb:'rgba(244,114,182,0.55)',
        title:'Theme Studio — themes restyle your whole blog (v1.15.0)',
        desc:'Applying a theme now transforms the entire public site, not just the colours. All nine built-in design themes — Gale, Zephyr, Dispatch, Vivid, Beacon, Ripple, Maverick, Agora and Apex — style the real public markup across every section: navigation, homepage hero, post cards (including cover images), article pages, the author box, related posts, comments, and a multi-column footer, each in its own visual language. Colour presets carry a layout archetype (Minimal / Classic / Magazine / Editorial / Bold) so even a palette swap changes structure and spacing. Logo, colours, fonts, layout, hero and the social/OG share image are all edited in one place with a live preview. CSP-safe, same-origin /theme.css, no inline styles, no CDNs (ADR-0086).',
        tags:['9 whole-site themes','layout archetypes','live preview'] },
      { icon:'✍️', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.55)',
        title:'The most powerful sovereign editor (v1.14.0)',
      ...
      { icon:'📈', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.55)',
        ...
        tags:['cookieless','no-PII','funnels & retention'] },
      { icon:'✉️', iconBg:'bg-sky-900/60 border border-sky-800/60',     orb:'rgba(56,189,248,0.5)',
        title:'VayuMail — native mail sovereignty (v1.8.0 + v1.9.0)',
        desc:'Own your communication. Outbound: RFC 6376 DKIM signing (relaxed/relaxed, RSA-SHA256), direct-to-MX delivery with opportunistic STARTTLS (no third-party relay), a durable SQLite retry queue, Maildir storage, and automatic MX/SPF/DKIM/DMARC record generation with live DNS health checks. Inbound (v1.9.0, opt-in): a pure-Go SMTP-receive server (no open relay) plus a minimal IMAP read server so standard clients like Thunderbird read the Maildir — with transparent PGP decryption on read. No Mox fork: pure Go on the standard library (ADR-0077/0078).',
        tags:['DKIM','direct-MX','IMAP inbox'] },
      { icon:'🔑', iconBg:'bg-violet-900/60 border border-violet-800/60',orb:'rgba(167,139,250,0.5)',
        title:'VayuPGP — end-to-end PGP privacy layer (v1.8.0)',
        desc:'Encryption by architecture, built on ProtonMail go-crypto (Apache-2.0). Ed25519 + Curve25519 keypairs are auto-generated when an account is created; private keys are AES-256-GCM encrypted at rest under a master-secret-derived key and are never logged. Full encrypt / decrypt / sign / verify / encrypt-and-sign, key rotation that preserves old messages, and a Web Key Directory (WKD) at /.well-known/openpgpkey/ so any GPG client can discover your keys (ADR-0076).',
        tags:['Ed25519','AES-256 at rest','WKD'] },
      { icon:'🛡️', iconBg:'bg-emerald-900/60 border border-emerald-800/60',orb:'rgba(52,211,153,0.5)',
        title:'VayuOS — native control layer + security watcher',
        desc:'The sovereignty brain inside the single binary: a typed event bus (creating an account auto-provisions a PGP keypair and a mailbox), an ordered boot orchestrator with graceful degradation, and a health monitor — surfaced at /os/vayuos (keys, mail queue, DNS health, mailboxes, security updates). An opt-in, privacy-default-off security-update watcher tracks upstream releases of the crypto dependencies powering VayuPGP/VayuMail and surfaces available patches, transmitting nothing about your site (ADR-0080).',
        tags:['event bus','/os/vayuos','security updates'] },
      { icon:'🎨', iconBg:'bg-pink-900/60 border border-pink-800/60',    orb:'rgba(244,114,182,0.5)',
        title:'Theme Studio Gallery — 20+ sovereign presets (v1.8.0)',
        desc:'A Tumblr-style visual gallery of 20+ built-in themes — including the new Gale (editorial magazine) and Zephyr (bright creative) layouts. A CSP-safe Pico bridge maps design tokens to the public site so a preset restyles everything instantly via same-origin /theme.css, with WCAG-AA contrast and ≥44px touch targets throughout. No inline styles, no CDNs, no build step.',
        tags:['20+ presets','Gale & Zephyr','WCAG-AA'] },
      { icon:'📬', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.5)',
        title:'VayuMail accounts — roles, Archive & search (v1.13.0)',
        desc:'Toward a Gmail-like inbox, still 100% sovereign. Admin-managed mailboxes now carry a role (Administrator, Editor, Author, Reviewer/read-only, or custom) set on creation and editable inline; create/delete stays admin-only. A first-class Archive folder joins Inbox/Sent/Drafts/Junk/Trash, and a bounded, fully-local full-text search scans From/To/Subject and bodies across folders — no external index, no extra service (ADR-0083).',
        tags:['role-based accounts','Archive','local search'] },
      { icon:'✍️', iconBg:'bg-pink-900/60 border border-pink-800/60',   orb:'rgba(244,114,182,0.5)',
        title:'Ghost-style block editor (v1.10.0)',
        desc:'Write naturally. Inline rich text — bold, italic, inline code, links and strikethrough — still sanitised by bluemonday, plus a selection toolbar and Markdown shortcuts (## heading, dash bullet, 1. numbered, > quote, fenced code, --- divider) converted as you type. Enter starts the next block, Shift+Enter a soft break, Backspace removes an empty block; a filterable slash menu inserts cards, and you can paste or drag-and-drop an image to upload it inline (ADR-0081).',
        tags:['inline rich text','markdown shortcuts','image paste'] },
      { icon:'🧩', iconBg:'bg-pink-900/60 border border-pink-800/60',   orb:'rgba(244,114,182,0.5)',
        title:'Theme Studio code editor + import/export (v1.11.0 / v1.12.0)',
        desc:'Tumblr-style theme control within the strict CSP. A Custom CSS editor served same-origin via /theme.css — no inline styles, no external origins, no scripts — declarative Head and SEO meta on a validated, escaped meta-tag allowlist (raw head HTML is rejected), and full theme import/export as a portable JSON envelope whose tokens are validated by compiling them before they go live (ADR-0082).',
        tags:['custom CSS','head meta','import/export'] },
      { icon:'📝', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.55)',
        title:'Draft/publish workflow — v1.7.0',
        desc:'New in v1.7.0 (ADR-0075): articles are first-class drafts or published posts. The VayuOS post manager (/os/posts) lists every article with a live status pill and one-click Publish / Unpublish that purges render caches immediately. Drafts are invisible on every public surface — API, article page, RSS, sitemap, search index, related posts and the comment API. Three security fixes (LEAK-1 Critical, LEAK-2 High, LEAK-3 Low) ensure a draft is indistinguishable from a missing article for any unauthenticated caller, and the on-disk render cache never contains draft HTML.',
        tags:['draft','publish','security'] },
      { icon:'🪟', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.45)',
        title:'VayuOS — unified operator powerhouse',
        desc:'As of v1.7.0 all operator tools live inside the VayuOS shell: the block editor, post manager, operator consoles (System Modes, Policy Provenance Inspector, Runtime Topology, Replay Explorer, Fault Engine, ADR Registry), Theme Studio, Governance, Tools & Plugins, Media Library, SEO, Analytics and Members. There is one admin — /os — and old /admin/* paths issue permanent (301) redirects. Single binary, zero CDNs, strict CSP with per-request nonces, no unsafe-eval, no unsafe-inline. Member/reader signup at /signup with optional Ghost-style Sign in / Sign up buttons on the public homepage, toggled from VayuOS Members settings.',
        tags:['/os','post manager','member signup'] },
      { icon:'🎬', iconBg:'bg-teal-900/60 border border-teal-800/60',   orb:'rgba(45,212,191,0.45)',
      ...
      { icon:'🔌', iconBg:'bg-orange-900/60 border border-orange-800/60',orb:'rgba(251,146,60,0.35)',
        ...
        desc:'Out-of-process subprocess plugins under a deny-by-default capability model — filesystem, network and write allowlists, seccomp, PID/IPC/mount namespaces and cgroup limits. A formal, language-neutral, versioned interface spec (docs/plugins/SPEC.md, ADR-0074) lets you author plugins in any language, with a live registry in the VayuOS Tools panel. Five worked examples including trace-tap and seo-stamp.',
        tags:['sandbox','capabilities','spec v1.0'] },
      ...
      { icon:'🧱', iconBg:'bg-teal-900/60 border border-teal-800/60',     orb:'rgba(45,212,191,0.45)',
        title:'Next-gen admin & block editor (/os)',
        desc:'A ground-up redesign (ADR-0068): a cohesive design system, an intelligent dashboard with a 14-day sparkline, and a typed block editor whose document is rendered server-side and re-sanitised — there is no raw-HTML escape hatch. Plus a media library, members management, native SEO and analytics, all on the same strict-CSP, zero-CDN stack. As of v1.6.0 this is the sole admin — the legacy Admin v2 has been removed (ADR-0069 Stage 3).',
      ...
      { icon:'🔐', iconBg:'bg-cyan-900/60 border border-cyan-800/60',     orb:'rgba(34,211,238,0.40)',
        ...
        desc:'RFC 6238 time-based one-time passwords implemented in pure Go standard library (validated against the RFC test vectors) — no third-party 2FA SDK. Two-step enrolment stores the secret disabled until a code is verified, so an abandoned setup can never lock an operator out. Enforced on the VayuOS sign-in surface.',
      ...
      { icon:'💳', iconBg:'bg-green-900/60 border border-green-800/60',   orb:'rgba(74,222,128,0.32)',
        title:'Premium memberships & paywalls',
        desc:'Turn readers into paying members with passwordless magic-link sign-in (no reader passwords stored). Define priced subscription tiers with monthly/yearly pricing and benefits, publish a themed pricing page, and give members a self-service portal to manage their plan and newsletter preference. Gate any article as public, members or paid — non-members get a tier-aware preview plus a sign-in CTA. Track Monthly Recurring Revenue, segment members with labels, and let an optional signature-verified Stripe webhook handle paid upgrades, with no payment SDK embedded. Run a team too: admin/editor/author roles with sovereign VayuMail mailboxes auto-provisioned for staff, and public author profiles (avatar, bio, social links) at /author/{id}.',
        tags:['multi-tier','member portal','team roles','MRR','Stripe-optional'] },
    ...
    screenshots: [
      ...
      { label:'Sign in',          path:'/os/login',         src:'screenshots/os-login.png',         caption:'VayuOS sign-in — the single front door to the control panel. Strict CSP, self-hosted, no third-party requests.' },
      { label:'Member signup',    path:'/signup',           src:'screenshots/member-signup.png',    caption:'Public reader/member signup — branded and passwordless; enter an email, get a one-time sign-in link. Zero third-party requests.' },
      { label:'VayuOS dashboard', path:'/os',               src:'screenshots/admin-os-dashboard.png', caption:'VayuOS — the single, fast admin at /os. Grouped sidebar, stat cards and a CSP-safe 14-day server-rendered sparkline (ADR-0068).' },
      { label:'Post manager',     path:'/os/posts',         src:'screenshots/admin-os-posts.png',    caption:'Post manager (v1.7.0) — all articles in one view with live status pills (Published / Draft) and one-click Publish / Unpublish that purges render caches immediately.' },
      { label:'Block editor',     path:'/os/editor',        src:'screenshots/admin-os-editor.png',  caption:'Typed block editor with opt-in AI-assist and inline version-history diff — document rendered server-side and re-sanitised, no raw-HTML escape hatch.' },
      { label:'Theme Studio',     path:'/os/theme',         src:'screenshots/admin-os-theme.png',   caption:'Native Theme Studio (v1.5.0) — preset gallery and design-token editor with an instant, CSP-clean live preview via scripted CSSOM. No inline styles, no CDNs.' },
      { label:'Governance',       path:'/os/governance',    src:'screenshots/admin-os-governance.png', caption:'Governance panel — system mode + transition lineage, error-budget ledger, and live policy-engine evaluation (pass/warn/fail). Server-rendered, CSP-clean.' },
      { label:'Tools & Plugins',  path:'/os/tools',         src:'screenshots/admin-os-tools.png',   caption:'Tools & Plugins — sovereign built-in modules plus a live registry of sandboxed out-of-process plugins (capability allowlists, seccomp, namespaces). See docs/plugins/SPEC.md.' },
      { label:'Monitoring',       path:'/os/monitoring',    src:'screenshots/admin-os-monitoring.png', caption:'Monitoring — live latency percentiles, storage/queue health and the governance budget ledger, polled from the same in-process sources as the JSON APIs.' },
      { label:'Media library',    path:'/os/media',         src:'screenshots/admin-os-media.png',   caption:'Media library — magic-number-validated uploads; SVG refused as an XSS vector.' },
      { label:'SEO',              path:'/os/seo',           src:'screenshots/admin-os-seo.png',     caption:'Native SEO — sitemap, robots, structured data and per-post readiness, all in VayuOS.' },
      { label:'Analytics',        path:'/os/analytics',     src:'screenshots/admin-os-analytics.png', caption:'Privacy-first analytics — cookieless, no PII, daily aggregates only.' },
      { label:'Security (2FA)',   path:'/os/security',      src:'screenshots/admin-os-security.png', caption:'Security — TOTP two-factor enrolment (RFC 6238, pure stdlib), enforced on sign-in.' },
      { label:'Policy modes',     path:'/os/modes',            src:'screenshots/policy-modes.png',     caption:'Six modes: normal → degraded → read-only → quarantined → recovery → maintenance.' },
      { label:'Policy inspector', path:'/os/policy', src:'screenshots/policy-inspector.png', caption:'Live error budgets with severity classification and actuation status.' },
      { label:'Runtime topology', path:'/os/topology', src:'screenshots/runtime-topology.png', caption:'Subsystem graph with health and dependency edges.' },
      { label:'Replay explorer',  path:'/os/replay',           src:'screenshots/replay-explorer.png',  caption:'Inspect and re-drive dead-letter activities from the SQLite outbox.' },
      { label:'Fault engine',     path:'/os/faults',           src:'screenshots/fault-manager.png',     caption:'Inject controlled faults to exercise recovery; watch escalation through the mode state machine. Non-production.' },
      { label:'ADR registry',     path:'/os/adr',              src:'screenshots/adr-registry.png',     caption:'Every architecture decision record, browsable inside VayuOS.' },
    ],
    ...
    principles: [
      ...
      { title:'Decisions have records',             body:'75 ADRs document every significant choice — from WAL checkpointing and the inbox replay protocol to the draft/publish security model. The codebase ships with its own archaeology.' },
    ],
    ...
    compareRows: [
      ...
      { f:'Native mail server in the binary (DKIM)', v:['yes','no','no','no'] },
      { f:'Inbound IMAP mailbox (read your mail)', v:['yes','no','no','no'] },
      { f:'End-to-end PGP encryption + WKD',     v:['yes','no','no','no'] },
      { f:'PGP keys auto-generated, encrypted at rest', v:['yes','no','no','no'] },
      { f:'Zero reader-side trackers / cookies', v:['yes','no','partial','no'] },
      { f:'Privacy-first analytics built in (no-PII)', v:['yes','plugin','partial','partial'] },
