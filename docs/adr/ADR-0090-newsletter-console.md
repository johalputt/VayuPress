# ADR-0090: Newsletter Console — Operator Page, Subscriber Management & Tracked Broadcasts
...
behaviour in a same-origin nonce-gated JS file, all dynamic values escaped).
...
3. **Tracked broadcasts.** A new `newsletter_broadcasts` table (migration 044)
...
- Migration `044-newsletter-broadcasts` is additive and backward-compatible; it
