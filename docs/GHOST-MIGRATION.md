...
| Full-text search (VayuFind) | < 30 ms |
...
## Search at 200k+ posts

Search is **built in** (VayuFind, [ADR-0101](adr/ADR-0101-builtin-search-vayufind.md)) —
one binary, no second service to install, secure, monitor or back up. There is
nothing to set up: the index is built and maintained by VayuPress itself.

To rebuild the index after a bulk import:
