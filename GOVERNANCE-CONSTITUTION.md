...
- **Licence changes**: the maintainer may change the outbound licence on future versions **without any contributor's permission** — Apache-2.0 §4 permits it and no CLA is needed for it. There is no contributor veto and no committee. Any such change is **forward-only, at a major version, and announced in advance**; contributors are told, not asked. Every published release remains available under the licence it shipped with — that grant is irrevocable and is never reinterpreted. `docs/LICENSING.md` records a standing pledge to make Apache-2.0 permanent once donations sustain the project.
