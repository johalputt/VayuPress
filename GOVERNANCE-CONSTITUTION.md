...
- **Licence changes**: the maintainer may change the outbound licence on future versions **without any contributor's permission** — Apache-2.0 §4 permits it and no CLA is needed for it. There is no contributor veto and no committee. **That freedom exists and will not be used**: `docs/LICENSING.md` records an unconditional pledge that VayuPress stays Apache-2.0 permanently — no funding condition, no adoption condition, no acquisition condition. Were any future maintainer to break it, the remedy needs nobody's permission: fork the last Apache-2.0 release. Every published release remains available under the licence it shipped with — that grant is irrevocable and is never reinterpreted. Any change that did occur would be **forward-only, at a major version, and announced in advance**; contributors are told, not asked.
