...
var catalogMeta = map[string]ThemeMeta{
	...
	"Maverick": {
		Tagline:     "Bold, creative statement theme.",
		Description: "A bold, creative statement theme for those who dare to stand out — a high-contrast monochrome canvas with a punchy magenta accent, big display typography, and sharp corners. Ships a complete component kit: full-bleed background-media heroes with overlaid text (configurable button style, content position and length), an 'our selection' slider, animated floating circles for testimonials or tags, creative/default post cards with eight image aspect ratios, four post templates (default and right-aligned, each with optional featured video), text-style and heading-scale controls, page-load animations, custom account pages with background video, and authors/FAQ/archive/all-posts pages — light & dark.",
		Tags:        []string{"bold", "creative", "portfolio", "animated", "layout"},
		Category:    CatCreative,
	},
	"Agora": {
		Tagline:     "Community & discussion, dual sidebars.",
		Description: "A clean, forum-inspired community and discussion theme on a neutral canvas with a friendly indigo-violet accent. Ships a complete component kit: a three-column layout with dual toggleable sidebars, a customizable hero (six image positions and three actions), homepage feed tabs (latest/featured/updated/categories) with four switchable view styles (list/compact/cards/articles), post cards with a comment-count badge and an inline comments drawer, topic/category sections and a tag-cards template, default and full-width post templates, membership pages (sign in/up, account, tiers), an authors page, a blank landing template, and a footer with a light/dark toggle.",
	...
	"Vayu": {
		...
		Tags:        []string{"flagship", "teal", "saffron", "cosmic", "gradient", "fast", "brand"},
		Category:    CatFlagship,
	},
}
