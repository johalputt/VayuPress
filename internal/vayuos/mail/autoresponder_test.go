...
func TestShouldAutoReplySafetyMatrix(t *testing.T) {
	...
	if got := shouldAutoReply(mbox, []byte(plain)); got != "alice@other.com" {
	...
	if got := shouldAutoReply(mbox, []byte(rt)); got != "team@other.com" {
		t.Fatalf("Reply-To should win, got %q", got)
	}
	...
	never := map[string]string{
		...
		"list-id":         "List-Id: <dev.lists.x.org>\r\nFrom: a@b.c\r\n\r\nx",
		"list-unsub":      "List-Unsubscribe: <mailto:u@x.y>\r\nFrom: a@b.c\r\n\r\nx",
		"forward-tag":     forwardLoopHeader + ": m@x.y\r\nFrom: a@b.c\r\n\r\nx",
		"mailer-daemon":   "From: MAILER-DAEMON@other.com\r\n\r\nx",
		"noreply":         "From: no-reply@other.com\r\n\r\nx",
		"self":            "From: " + mbox + "\r\n\r\nx",
		"no-from":         "Subject: only\r\n\r\nx",
	}
	for name, msg := range never {
		if got := shouldAutoReply(mbox, []byte(msg)); got != "" {
			t.Errorf("%s: must not answer, got %q", name, got)
		}
	}
	// Auto-Submitted: no is explicitly answerable (RFC 3834).
	asNo := "Auto-Submitted: no\r\nFrom: a@other.com\r\n\r\nx"
	if got := shouldAutoReply(mbox, []byte(asNo)); got != "a@other.com" {
...
func TestAutoReplyEndToEnd(t *testing.T) {
	...
	if _, err := e.DeliverInbound("ankush@example.com", raw); err != nil {
	...
	if _, err := e.DeliverInbound("ankush@example.com", loop); err != nil {
