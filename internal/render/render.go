...
// AuthorInfoFn optionally resolves a display author name to its public profile
// slug and avatar URL, so the article byline can link to /author/<slug> with a
...
// queue, so the form reports "awaiting moderation" on success. A signed-in
// commenter can edit or delete their own comment inline, and an operator can
// delete any comment (moderation); the server authorises every such action by
// session, never trusting client-sent flags. Each comment shows a real
// self-hosted SVG country flag (/os/static/flags/<cc>.svg) rather than an emoji.
const CommentsJS = `(function(){
  ...
  // Web addresses in a comment become links. Only http(s) addresses are matched,
  // so a "javascript:" or "data:" address stays text, and one that does not
  // parse stays text too. Built with createElement and
  // textContent, so nothing a commenter types can become markup. rel=ugc and
  // nofollow tell search engines a visitor posted it, so a spam link earns
  // nothing; there is no noreferrer, so the linked site still sees the visit
  // came from here (the site's Referrer-Policy sends the origin, never the page).
  // Sentence punctuation after an address, and an unbalanced closing
  // parenthesis, belong to the sentence, not the link.
  function fillBody(el,text){
    while(el.firstChild)el.removeChild(el.firstChild);
    var s=esc(text),re=/https?:\/\/[^\s<>"']+/g,last=0,m;
    while((m=re.exec(s))){
      var raw=m[0],trail='';
      while(raw.length){
        var ch=raw.charAt(raw.length-1);
        if(ch===')'){if((raw.match(/\)/g)||[]).length<=(raw.match(/\(/g)||[]).length)break;}
        else if('.,;:!?]'.indexOf(ch)<0)break;
        trail=ch+trail;raw=raw.slice(0,-1);
      }
      if(m.index>last)el.appendChild(document.createTextNode(s.slice(last,m.index)));
      var u=null;try{u=new URL(raw);}catch(e){}
      if(u&&u.hostname){
        var a=document.createElement('a');a.href=u.href;a.textContent=raw;
        // A link to this site is an internal link: same tab, and no nofollow,
        // which would throw away the site's own internal linking.
        if(u.host!==location.host){a.rel='ugc nofollow noopener';a.target='_blank';}
        el.appendChild(a);
      }else{el.appendChild(document.createTextNode(raw));}
  ...
  // tooltip; a machine-readable ISO for the <time datetime>. flagEl() builds a
  // real, self-hosted SVG flag <img> (/os/static/flags/<cc>.svg — same-origin, so
  // img-src 'self' covers it on clearnet and .onion alike) for an ISO alpha-2
  // country code, falling back to a neutral globe when the code is absent,
  // malformed or has no flag. This replaces the old regional-indicator emoji,
  // which Windows renders as bare letters ("IN") instead of a flag.
  ...
  // PATCH the new text, then restore the paragraph. bodyEl.hidden (a DOM property,
  // not an inline style) keeps this CSP-safe under style-src 'self'.
  function startEdit(el,bodyEl,c){
    if(el.querySelector('.vayu-comment-editform'))return;
    var ef=document.createElement('form');ef.className='vayu-comment-editform';ef.setAttribute('novalidate','');
    var ta=field('Edit your comment…','textarea',true);ta.value=c.body||'';
    var save=document.createElement('button');save.type='submit';save.className='vayu-comment-submit';save.textContent='Save';
    var cancel=document.createElement('button');cancel.type='button';cancel.className='vayu-comment-act';cancel.textContent='Cancel';
    var st=document.createElement('span');st.className='vayu-comment-status';st.setAttribute('role','status');
    var acts=document.createElement('div');acts.className='vayu-comment-actions';acts.appendChild(save);acts.appendChild(cancel);acts.appendChild(st);
    ef.appendChild(ta);ef.appendChild(acts);
    function close(){if(ef.parentNode)ef.parentNode.removeChild(ef);bodyEl.hidden=false;}
    cancel.addEventListener('click',close);
    ef.addEventListener('submit',function(e){e.preventDefault();var nv=ta.value.trim();if(!nv){st.textContent='Please write something.';return;}
      save.disabled=true;st.textContent='Saving…';
      fetch('/api/v1/articles/'+encodeURIComponent(slug)+'/comments/'+encodeURIComponent(c.id),{method:'PATCH',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({body:nv})})
        .then(function(r){return r.json().then(function(d){return{ok:r.ok,status:r.status,d:d};});})
        .then(function(res){save.disabled=false;
          if(res.ok){c.body=nv;fillBody(bodyEl,nv);close();}
          else if(res.status===401){st.textContent='Please sign in.';}
          else{st.textContent=(res.d&&res.d.error&&(res.d.error.message||res.d.error))||'Could not save.';}})
        .catch(function(){save.disabled=false;st.textContent='Could not save.';});});
    bodyEl.hidden=true;el.insertBefore(ef,bodyEl.nextSibling);ta.focus();
  }
  function doDelete(id){
    if(!window.confirm('Delete this comment? This cannot be undone.'))return;
    fetch('/api/v1/articles/'+encodeURIComponent(slug)+'/comments/'+encodeURIComponent(id),{method:'DELETE',credentials:'same-origin'})
      .then(function(r){if(r.ok){load();}else if(r.status===401){if(typeof window.vpPortalOpen==='function'){window.vpPortalOpen('signin');}}else{window.alert('Could not delete the comment.');}})
      .catch(function(){window.alert('Could not delete the comment.');});
  }
  function post(parentId,text,onResult){
    return fetch('/api/v1/articles/'+encodeURIComponent(slug)+'/comments',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({body:text,parent_id:parentId||''})})
      .then(function(r){return r.json().then(function(d){return{ok:r.ok,status:r.status,d:d};});}).then(onResult);
  }
  function commentForm(parentId,placeholder,submitLabel,onPosted){
    var form=document.createElement('form');form.className='vayu-comment-form';form.setAttribute('novalidate','');
    var bodyI=field(placeholder,'textarea',true);
    var btn=document.createElement('button');btn.type='submit';btn.className='vayu-comment-submit';btn.textContent=submitLabel;
    var status=document.createElement('span');status.className='vayu-comment-status';status.setAttribute('role','status');
    var actions=document.createElement('div');actions.className='vayu-comment-actions';actions.appendChild(btn);actions.appendChild(status);
    form.appendChild(bodyI);form.appendChild(actions);
  ...
  function card(c,isReply){
    ...
    // A real self-hosted SVG flag when geo was captured; a neutral globe
    // otherwise, so a comment always carries a location cue that renders on every
    // platform (including Windows, where the old emoji flag showed as letters).
    line.appendChild(flagEl(c.country));
    ...
    var body=document.createElement('p');body.className='vayu-comment-body';fillBody(body,c.body);
    el.appendChild(meta);el.appendChild(body);
    // Action bar: Reply (signed-in, top-level only), plus Edit/Delete when the
    // server says this viewer may (own comment → edit+delete; operator → delete
    // any). Only rendered when at least one action applies, so guest views stay clean.
    var canReply=me&&!isReply,canEdit=!!c.can_edit,canDelete=!!c.can_delete;
    if(canReply||canEdit||canDelete){
      ...
      el.appendChild(bar);el.appendChild(holder);
    }
    return el;
  }
  function render(items){
    ...
    items.forEach(function(c){
      ...
      var el=card(c,false);
      var rs=kids[c.id];
      if(rs&&rs.length){var wrap=document.createElement('div');wrap.className='vayu-comment-replies';rs.forEach(function(r){wrap.appendChild(card(r,true));});el.appendChild(wrap);}
  ...
  function memberForm(name){clear(compose);
    ...
    compose.appendChild(who);compose.appendChild(commentForm('','Write a comment…','Post comment',null));}
  fetch('/api/v1/members/me',{credentials:'same-origin',headers:{'Accept':'application/json'}})
    .then(function(r){return r.ok?r.json():null;})
    .then(function(d){if(d&&d.authenticated&&d.member){me=d.member;memberForm(d.member.name);}else{guestPrompt();}load();})
    .catch(function(){guestPrompt();load();});
})();`
...
type articlePage struct {
	...
	Author              string
	AuthorBio           string
	AuthorSlug          string // public profile handle → /author/<slug>
	AuthorAvatar        string // author avatar URL (empty → initials placeholder)
...
{{if and .Author (not .IsPage)}}<div class="vayu-byline" itemprop="author" itemscope itemtype="https://schema.org/Person">
  ...
  {{if .AuthorAvatar}}<img class="vayu-byline-avatar" src="{{.AuthorAvatar}}" alt="" width="36" height="36" loading="lazy">{{else}}<span class="vayu-byline-avatar vayu-byline-avatar--ph" aria-hidden="true">{{.Author | initial}}</span>{{end}}
  <span class="vayu-byline-name" itemprop="name">{{.Author}}</span>
  {{if .AuthorSlug}}</a>{{end}}
</div>{{end}}
