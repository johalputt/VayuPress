...
// the browser then reports "example.test:2052" as the referrer. That is the site
...
func TestReferrerHostDropsThePort(t *testing.T) {
	for _, tc := range []struct{ in, want string }{
		{"https://example.test:2052/some-post", "example.test"},
		{"https://www.example.test:2052/", "www.example.test"},
		{"https://example.test/some-post", "example.test"},
...
func TestSelfHostPatternsCoverThePortForms(t *testing.T) {
	...
	if host != "example.test" {
		t.Fatalf("host = %q, want %q (the helper must lower-case the domain)", host, "example.test")
	}
	for _, tc := range []struct{ name, got, want string }{
		{"subdomain", sub, "%.example.test"},
		{"host:port", hostPort, "example.test:%"},
		{"subdomain:port", subPort, "%.example.test:%"},
	} {
		if tc.got != tc.want {
	...
	if !strings.HasPrefix("example.test:2052", strings.TrimSuffix(hostPort, "%")) {
		t.Errorf("%q would not exclude the observed self-referral %q", hostPort, "example.test:2052")
	}
	if !strings.Contains("www.example.test:2052", strings.Trim(subPort, "%")) {
		t.Errorf("%q would not exclude the observed self-referral %q", subPort, "www.example.test:2052")
	}
}
